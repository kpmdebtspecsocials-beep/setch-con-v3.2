...
// Search wards endpoint for address autocomplete
router.get('/search', async (req, res) => {
  try {
    ...
    // Search wards by name with municipality info
    const { data: wards, error } = await supabase
      ...
      `)
      .or(`name.ilike.%${query}%,ward_id.ilike.%${query}%`)
      .order('name')
      .limit(parseInt(limit));

    if (error) {
      return res.status(400).json(formatError('Failed to search wards'));
    }

    // Format results for address input
    const formattedWards = wards.map(ward => ({
      ...
      // Add approximate coordinates if available in properties
      lat: ward.properties?.centroid?.lat || null,
      lng: ward.properties?.centroid?.lng || null,
    }));

    res.json(formatSuccess({ wards: formattedWards }));
